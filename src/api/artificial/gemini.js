...
let res = new Bard({
    "__Secure-1PSID":
        "g.a000wgiwdIVQITRNiVhW2SJVYNeI9KjA9VmrMvr2aJJbGpBk1Hf-cRcH1CNDvjCkPWZGFWH12wACgYKAT0SARMSFQHGX2MisITQLoalHF33WMR1hYAkKhoVAUF8yKpZnTDyMOrUt032KKikV_Ow0076"
});
...
async function ask(
    ...
    buffer,
    ids = {
        conversationID: "c_39267d50a1c58e4f",
        responseID: "r_924548455fad10aa",
        choiceID: "rc_43e169ee22025055",
        _reqID: "200000"
    }
) {
    try {
        if (buffer) {
            const result = await res.ask(prompt, {
                format: Bard.JSON,
                image: Buffer.from(buffer, "base64"),
                ids: ids
            });
            return { status: true, message: result.content, ids: result.ids };
        } else {
            const result = await res.ask(prompt, {
                format: Bard.JSON,
                ids: ids
